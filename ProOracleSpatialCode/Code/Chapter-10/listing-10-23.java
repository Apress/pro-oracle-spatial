...
// Construct input node array
Node[] inputNodes = new Node[nodeIds.length];
for (int i = 0; i < nodeIds.length; i++)
  inputNodes[i] = network.getNode(nodeIds[i]);

// Display the visitation order
Node[] outputNodes = NetworkManager.tspOrder(inputNodes, tspPath);
System.out.println (" Actual node visitation order : " );
for (int i = 0; i < outputNodes.length; i++)
  System.out.println (" Node " + outputNodes[i].getID() + "\t" +
    outputNodes[i].getName());
