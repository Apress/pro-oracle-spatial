<!-- Listing 12-16. Example Application -->
<!--
...
- show the shortest route from the location mark to the nearest customer (or branch
  or competitor).
...
<%
  ...
  // Constants
  int    mapWidth = 350;              // Map width in pixels
  int    mapHeight = 300;             // Map height in pixels
  String defDataSource = "SPATIAL10G";// Default data source name
  String defBaseMap = "US_CITY_MAP";  // Default map name
  String defNetwork = "NET_SF";       // Default network name
  double defCx = -122.43302833333328; // Default origin longitude
  ...
  String[] colsToSelect               // Columns to select for application themes
    = new String[]{
      ...
      "STATE",
      "PHONE_NUMBER",
      "GET_POINT_X(LOCATION) LONGITUDE",
      "GET_POINT_Y(LOCATION) LATITUDE"
    };
  ...
  String networkName = null;          // Name of network for network-based searches
  ...
  boolean showRoute;                  // Show route to nearest neighbor
  ...
  showRoute = request.getParameter("showRoute") != null ?
    Boolean.valueOf(request.getParameter("showRoute")).booleanValue(): false;

  // - Within-distance search parameters
  distSearchParam = request.getParameter("distSearchParam") != null ?
    Double.valueOf(request.getParameter("distSearchParam")).doubleValue() : 0;
  ...
  // Reload Network from session (if present)
  Network net = (Network) session.getAttribute("NetworkHandle");
  ...
  if (userAction != null) {
    ...
    // -----------------------------------------------------------------------
      ...
      // Remove any route from the current mark
      mv.removeAllLinearFeatures();
...
<title>Essential Guide to Oracle Spatial - Sample Application</title>
...
<table border="1" cellpadding="0" cellspacing="0">
  <tr>
    <td width="260" valign="top" align="left">
      ...
      <table border="0">
        ...
        <!-- Find Address -->
        <tr><td>
          <font color="#449922"><b>Find address:</b></font>
        </td></tr>
        <tr>
          <td align="left">
            <textarea
              name="findAddress" rows="3" cols="30"
        ...
        <!-- Show Location mark -->
        <tr><td>
          <font color="#449922"><b>Location Mark:</b></font>
        </td></tr>
        <tr>
          <td>
          <table>
        ...
        <tr><td>
          <font color="#449922"><b>Search from Location Mark</b></font>
        </td></tr>
        <tr><td><font size="-1"><b>
        ...
        <tr><td>
          <font color="#449922"><b>Find nearest from Location Mark</b></font>
        </td></tr>
        <tr><td><font size="-1"><b>
        ...
        <tr><td>
          <B>Show route to nearest</B>
          <input type="checkbox" name="showRoute" value="true"
            <%= showRoute?"checked":""%>
          >
        </td></tr>
        <tr><td align="center">
      ...
      <table border="1" cellpadding="0" cellspacing="10">
        ...
        <tr>
          <td>
            <table border="0">
              <tr>
                <td width="100"><font color="#449922">
                  <b>Click on the map to:</b> </font></td>
                <td>
      ...
      <table>
        ...
        <tr>
          ...
          <% if (featureInfo !=null && featureInfo.length>0) {%>
            <tr><td><font color="#449922"><b>Identification result(s):
          ...
          <% if (geocodeInfo !=null && geocodeInfo.length>0) {%>
            <tr><td><font color="#449922"><b>Address found:</b>
            </font></td></tr>
            <tr><td>
            <table border="0">
 ...
 <!-- Error messages  -->
 <tr><td colspan="3"><font color="#FF0000"><b><%= mapError %></b></font></td></tr>
